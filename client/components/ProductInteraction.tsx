				...
				<div className="flex items-center gap-2">
					{product.sizes.map((size) => (
						<div
							...
							<div
								className={`w-6 h-6 text-center flex items-center justify-center font-medium text-sm ${selectedSize === size ? "bg-black text-white" : "bg-white text-black"}`}>
								{size.toUpperCase()}
							</div>
						</div>
					))}
			...
			<div className="flex flex-col gap-2 text-sm">
				...
				<div className="flex items-center gap-2">
					{product.colors.map((color) => (
						<div
							className={`cursor-pointer border p-0.5 ${selectedColor === color ? "border-gray-600" : "border-gray-300"}`}
							key={color}
